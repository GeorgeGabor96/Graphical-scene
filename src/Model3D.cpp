...
namespace gps {
    ...
    Model3D::Model3D(std::vector<GLuint> indices, std::vector<gps::Vertex> vertices, std::vector<str::TextureInfo> texturesInfo)
    {
        std::vector<gps::Texture> textures;
        gps::Texture currentTexture;

        int texturesInfoSize = texturesInfo.size();
        for (int i = 0; i < texturesInfoSize; i++)
        {
            currentTexture = LoadTexture(texturesInfo[i].path, texturesInfo[i].type);
